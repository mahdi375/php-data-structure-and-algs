...
class WeightGraph
{
    ...
    public function hasCycle(): bool
    {
        ...
        foreach($this->nodes as $label => $node) {
            ...
            [$visited, $hasCycle] = $this->hasCycleRecursion(null, $node, $visited, $hasCycle);
        }

        return $hasCycle;
    }

    private function hasCycleRecursion($parent = null, $node, $visited, $hasCycle)
    {
        if(in_array($node->getLabel(), $visited)) {
            $hasCycle = true;
        }

        if($hasCycle) return [$visited, $hasCycle];
        ...
        foreach($node->getEdges() as $label => $edge) {
            ...
            [$visited,  $hasCycle] = $this->hasCycleRecursion($node->getLabel(), $this->nodes[$edge->to], $visited, $hasCycle);
        }
